(function () {
  ...
  /* ── Logout ── */
  function initLogout() {
    var logoutButton = qsOptional("#logout");
    if (!logoutButton) {
      console.warn("Logout button #logout not found; logout will be unavailable.");
      return;
    }
    ...
    var sideLogout = qsOptional('a.side-link.danger[href="#logout"]');
    if (sideLogout) {
  ...
  function initSideNav() {
    qsa(".side-nav .side-link").forEach(function (link) {
      ...
      if (!href || href.charAt(0) !== "#" || href === "#logout") return;
